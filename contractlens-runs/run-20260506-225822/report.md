...
|---|---|---|---|---|---:|
| Frontend Analyst | Extract frontend API expectations | `scan_frontend_contracts` | root=C:\dev\projects\contract-lens\examples\sample_project | Detected 2 frontend API calls | 16.52 |
| Backend Analyst | Extract backend routes/DTOs | `scan_backend_routes` | root=C:\dev\projects\contract-lens\examples\sample_project | Detected 2 backend routes | 18.68 |
| Contract Reviewer | Compare contracts (+ OpenAPI drift) | `compare_contracts` | fe=2 be=2 | 9 mismatch(es); openapi drift=3 | 0.65 |
| Report Writer | Build Markdown audit report | `build_markdown_report` | path=C:\dev\projects\contract-lens\contractlens-reports\cont | 45903 chars | 472.34 |
...
| `openapi_vs_code_response_schema` | Medium | OpenAPI fields: createdAt, name | Implementation fields: (none); only-in-spec (normalized): createdAt, name | Refresh the OpenAPI response schema to match serialized DTOs or rename backend properties / configure serializers so documentation reflects runtime JSON. |
...
| `openapi_vs_code_response_schema` | Medium | OpenAPI fields: id, thumbnailUrl | Implementation fields: (none); only-in-spec (normalized): id, thumbnailUrl | Refresh the OpenAPI response schema to match serialized DTOs or rename backend properties / configure serializers so documentation reflects runtime JSON. |
