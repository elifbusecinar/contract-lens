...
- [LangGraph] analyze_documentation: doc_claims=7 documentation_drift_rows=4
...
|---|---|---|---|---|---:|
| Frontend Analyst | Extract frontend API expectations | `scan_frontend_contracts` | root=C:\dev\projects\contract-lens\examples\sample_project | Detected 2 frontend API calls | 14.22 |
| Backend Analyst | Extract backend routes/DTOs | `scan_backend_routes` | root=C:\dev\projects\contract-lens\examples\sample_project | Detected 2 backend routes | 15.3 |
| Contract Reviewer | Compare contracts (+ OpenAPI drift) | `compare_contracts` | fe=2 be=2 | 9 mismatch(es); openapi drift=3 | 0.18 |
| Report Writer | Build Markdown audit report | `build_markdown_report` | path=C:\dev\projects\contract-lens\contractlens-reports\cont | 23254 chars | 361.7 |
...
| Docs/json mention field `thumbnailUrl` (known mismatch vs `thumbnail_path`) (`docs/API.md`:11) | Scanned contracts carry `thumbnail_path` but not `thumbnailUrl` in payload hints. | Medium | Align docs with `thumbnail_path` or change responses to emit `thumbnailUrl`. |
