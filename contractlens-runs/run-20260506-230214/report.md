...
- **effective:** full scan (not inside a Git worktree or `git` unavailable — changed-only ignored)
- **changed paths count (intersecting project scan):** _ignored — changed-only disabled under root (not Git)_

### Files considered

- `backend/ProjectsController.cs`
...
- Not a Git repository (or `git` is unavailable); cannot list changed files.
...
- [LangGraph] apply_git_changed_filter: changed_only=True narrow_openapi=False scan_files=6
- [LangGraph] git note: Not a Git repository (or `git` is unavailable); cannot list changed files.
- [LangGraph] git note: changed-only mode ignored: analyzing full discovered file list (not a Git repository or git failed).
...
|---|---|---|---|---|---:|
| Frontend Analyst | Extract frontend API expectations | `scan_frontend_contracts` | root=C:\dev\projects\contract-lens\examples\sample_project | Detected 2 frontend API calls | 15.98 |
| Backend Analyst | Extract backend routes/DTOs | `scan_backend_routes` | root=C:\dev\projects\contract-lens\examples\sample_project | Detected 2 backend routes | 15.77 |
| Contract Reviewer | Compare contracts (+ OpenAPI drift) | `compare_contracts` | fe=2 be=2 | 9 mismatch(es); openapi drift=3 | 0.44 |
| Report Writer | Build Markdown audit report | `build_markdown_report` | path=C:\dev\projects\contract-lens\contractlens-reports\cont | 55959 chars | 372.7 |
