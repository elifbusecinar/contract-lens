...
Optional reachability check (not a substitute for integration tests). **GET** only; redirects may be followed by the client; TLS/certificate validation uses system defaults.

- **URL:** `http://127.0.0.1:9`
- **Reachable (HTTP layer):** **no**
- **HTTP status:** _none (connection failed before response)_
- **Round-trip:** `2060` ms
...
- [LangGraph] runtime_probe: GET 'http://127.0.0.1:9' ok=False status=None
...
|---|---|---|---|---|---:|
| Frontend Analyst | Extract frontend API expectations | `scan_frontend_contracts` | root=C:\dev\projects\contract-lens\examples\sample_project | Detected 2 frontend API calls | 15.68 |
| Backend Analyst | Extract backend routes/DTOs | `scan_backend_routes` | root=C:\dev\projects\contract-lens\examples\sample_project | Detected 2 backend routes | 16.96 |
| Contract Reviewer | Compare contracts (+ OpenAPI drift) | `compare_contracts` | fe=2 be=2 | 9 mismatch(es); openapi drift=3 | 0.64 |
| Report Writer | Build Markdown audit report | `build_markdown_report` | path=C:\dev\projects\contract-lens\contractlens-reports\cont | 73467 chars | 568.44 |
...
- **Current run id:** `run-20260512-002617`
