...
|---|---|---|---|---|---:|
| Frontend Analyst | Extract frontend API expectations | `scan_frontend_contracts` | root=C:\dev\projects\contract-lens\examples\sample_project | Detected 2 frontend API calls | 13.73 |
| Backend Analyst | Extract backend routes/DTOs | `scan_backend_routes` | root=C:\dev\projects\contract-lens\examples\sample_project | Detected 2 backend routes | 16.3 |
| Contract Reviewer | Compare contracts (+ OpenAPI drift) | `compare_contracts` | fe=2 be=2 | 9 mismatch(es); openapi drift=3 | 0.19 |
| Report Writer | Build Markdown audit report | `build_markdown_report` | path=C:\dev\projects\contract-lens\contractlens-reports\cont | 80523 chars | 432.17 |
...
- **Current run id:** `run-20260512-003437`
...
- **GitHub REST:** optional `python -m contractlens.main ... --github-create-issue` posts Issues via PAT (`GITHUB_TOKEN`); `--github-issue-comment N` posts timeline comments on issues or PRs.
- **Scan cache:** optional `.contractlens/scan-cache/` entries keyed by file fingerprint (`mtime_ns`, `size`); enable `--scan-cache` or `[contractlens] scan_cache` — stale fingerprints refresh automatically.
